...
由于基于CNN的特征高层次抽象和在目标周围采样多个正样本的数据增强策略，网络有时无法找到包围目标的紧密边界框。我们应用Bounding Box Regression回归技术（该技术在目标检测很流行）来提高目标定位精度。给定测试序列的第一帧，我们训练一个简单的线性回归模型，利用目标位置附近样本的conv3特征预测精确的目标位置。在随后帧中，我们调整Eq估计的目标位置。如果被估计的目标是可靠的，就使用线性回归模型。边界框回归器仅在第一帧中训练，因为在线更新耗时，并且考虑到其风险，回归模型的增量学习可能没有多大帮助。参考[12]了解详细信息，因为我们使用相同的配方和参数。

## Implementation Details

通过多域学习来预训练1~5层的权重，对于新的序列随机初始化第6层的权重

只有4\~6的完全连接层的权重需要在线更新，在整个跟踪过程中1\~ 3层卷积层的权重是固定的。

这种策略不仅有利于提高计算效率，而且通过保留与域无关的信息避免过拟合。
...
为了在每个帧中生成候选目标，我们在平移和缩放维度中绘制了（=256）个来自高斯分布的样本，均值为上一个目标状态，协方差为对角矩阵。

r是上一帧中目标宽度和高度的平均值。通过将1.05乘以初始目标比例来计算每个候选边界框的比例。
